...
import { X, CreditCard, Wallet, Banknote, Landmark, Save, CheckCircle2, AlertCircle } from 'lucide-react';
...
const EditWalletModal = ({ isOpen, onClose, onSuccess, wallet }: EditWalletModalProps) => {
  ...
  const walletTypes = [
    ...
    { id: 'digital', name: 'Digital', icon: <Wallet size={20} /> },
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="glass-card w-full max-w-md rounded-3xl p-8 relative animate-in fade-in zoom-in duration-300 border border-white/10">
        ...
        <h2 className="text-2xl font-bold text-text-primary mb-6 tracking-tight">Edit Payment Method</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Input 
            label={`Current Balance (${profile?.currency || 'USD'})`} 
            type="number"
            step="0.01"
            placeholder="0.00"
          ...
          <Button type="submit" loading={loading} className="w-full py-4 text-lg gap-2 glow-secondary">
